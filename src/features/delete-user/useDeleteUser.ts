import { useState, useCallback } from 'react';
import { useDeleteUserMutation } from 'entities/user';

type DeleteState = { userId: number | null; error: string | null };

export const useDeleteUser = (onSuccess?: () => void) => {
  const [deleteUser, { isLoading }] = useDeleteUserMutation();
  const [state, setState] = useState<DeleteState>({ userId: null, error: null });

  const confirmDelete = useCallback(async () => {
    if (state.userId === null) return;

    try {
      await deleteUser(state.userId).unwrap();
      setState({ userId: null, error: null });
      onSuccess?.();
    } catch (err) {
      const message =
        err && typeof err === 'object' && 'message' in err
          ? String(err.message)
          : 'Не удалось удалить пользователя';
      setState({ userId: null, error: message });
    }
  }, [deleteUser, state.userId, onSuccess]);

  const requestDelete = useCallback((id: number) => {
    setState({ userId: id, error: null });
  }, []);

  const cancelDelete = useCallback(() => {
    setState({ userId: null, error: null });
  }, []);

  return {
    userIdToDelete: state.userId,
    requestDelete,
    confirmDelete,
    cancelDelete,
    isLoading,
    error: state.error,
  };
};
